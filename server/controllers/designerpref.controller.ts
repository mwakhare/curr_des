...
/**
 * DesignerPrefController class is for RESTful designer API using CRUD operations
 *
 */
export class DesignerPrefController 
{
	...
	*/
    ...
    public updateDesignerPref (req, res) 
	{
		/* req.user object is already in memory 
		because of above load() method*/ 
		let objDesigner: Designer = req.loadedDesigner;  
		...
		let tempPref: DesignerPref = {
			language: '',
    		country: '',
			currency: ''
		};
		console.log ("before updateDesignerPref- tempPref = " + util.inspect(tempPref));
		//console.log ("before updateDesignerPref- objDesigner.Pref.language = " + objDesigner.Pref.language);
		...
		objDesigner.updatePreference( (err, result) => {
			if( err ) 
				return res.status(400).json (
						{
							success: false,
